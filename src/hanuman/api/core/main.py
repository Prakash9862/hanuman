...
def create_app() -> FastAPI:
    ...
    @app.get("/status")
    def status():
        keys = ["NOTION_TOKEN","NOTION_VERSION","NOTION_PARENT_ID",
                "OBSIDIAN_VAULT_DIR","GITHUB_TOKEN","OPENAI_API_KEY",
                "GOOGLE_CLIENT_ID","GOOGLE_CLIENT_SECRET","GOOGLE_REDIRECT_URI"]
        env = {k: (os.getenv(k) is not None) for k in keys}
        return {"env": env, "mounted": sorted(list(app.router.routes_map.keys())) if hasattr(app.router, "routes_map") else "n/a"}

    # ====== Montage des routeurs connus (best-effort) ======
    # Orchestrations
    try_include(app, "hanuman.api.orchestrations.obsidian_to_notion")   # /obsidian/*
    try_include(app, "hanuman.api.orchestrations.github_sync_notion")   # /github_sync_notion/* (si existant)
