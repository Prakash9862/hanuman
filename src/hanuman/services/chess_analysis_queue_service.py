...
def start_analysis_queue(depth: int = 12, limit: int | None = 25) -> dict[str, Any]:
    ...
    with _LOCK:
        if _WORKER is not None and _WORKER.is_alive():
            return {"ok": False, "message": "Une analyse Stockfish est déjà en cours", "state": get_analysis_queue_status()}
        ...
        _WORKER.start()
    return {"ok": True, "message": f"Analyse lancée pour {len(pending)} partie(s)", "state": get_analysis_queue_status()}


def stop_analysis_queue() -> dict[str, Any]:
    if _WORKER is None or not _WORKER.is_alive():
        return {"ok": False, "message": "Aucune analyse en cours", "state": get_analysis_queue_status()}
    _STOP_EVENT.set()
